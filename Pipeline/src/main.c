...
void	print_result(int i, COUNT counter, REG *R)
{
	...
	printf("# of predicted branch:\t\t%d\n", counter.num_predict_O);
	printf("# of not predicted branch:\t%d\n", counter.num_predict_X);
}
...
int	main(int argc, char* argv[])
{
	...
	while (1)
	{
		i++;
		printf("\n============%dth cycle start.============\n\n", i);
		fetch(&ifid_latch[1], &ifid_latch[0], &idex_latch[1], &exmem_latch[1], btb, MEM);
		...
		memory_access(&exmem_latch[1], &memwb_latch[0], &memwb_latch[1], MEM, R, &counter);
